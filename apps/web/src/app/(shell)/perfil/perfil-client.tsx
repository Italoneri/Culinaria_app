...
const FALLBACK_AVATAR = 'https://images.unsplash.com/photo-1438761681033-6461ffad8d80?w=400&q=80&auto=format&fit=crop';
  ...
  return (
    <div data-testid="profile-screen" style={{ minHeight: '100dvh', overflowY: 'auto', overflowX: 'hidden', background: T.bg, paddingBottom: 110 }}>
      ...
      <div style={{ padding: '28px 24px 0', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        {editMode ? (
          <div style={{ width: '100%', maxWidth: 300, marginTop: 18, display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            {saveError && <div style={{ fontSize: 12, color: T.amber, fontWeight: 600 }}>{saveError}</div>}
            <button onClick={handleSave} disabled={saving} style={{
              background: T.amber, border: 'none', borderRadius: 12, padding: '12px', color: '#0D0D0D',
              fontFamily: T.sans, fontSize: 14, fontWeight: 700, cursor: saving ? 'not-allowed' : 'pointer',
          ...
          <>
            ...
            <div data-testid="profile-email" style={{ fontSize: 13, color: T.textMuted, marginTop: 4, fontWeight: 500 }}>
              {user?.email} · <span data-testid="profile-city">{editCity}</span>
            </div>
            {editBio && (
      ...
      <div style={{ padding: '28px 24px 0', display: 'flex', gap: 10 }}>
        <StatCard value={String(initialProfile?.stats.recipes_created ?? 0)} label="Receitas criadas" testid="stat-created" />
        <StatCard value={String(initialProfile?.stats.favorites ?? 0)} label="Favoritas" accent testid="stat-favorites" />
        <StatCard value="—" label="Cozinhadas" testid="stat-cooked" />
      </div>
      ...
      {initialFavorites.length === 0 ? (
        <div style={{ padding: '0 24px', textAlign: 'center', color: T.textDim, fontSize: 13, fontWeight: 500, paddingTop: 8 }}>
        ...
        <div data-testid="favorites-grid" style={{ padding: '0 24px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          {initialFavorites.slice(0, 4).map(r => (
